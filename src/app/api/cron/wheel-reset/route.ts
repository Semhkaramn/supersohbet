import { NextRequest, NextResponse } from 'next/server'
import { prisma } from '@/lib/prisma'
import { notifyWheelReset } from '@/lib/notifications'

export async function POST(request: NextRequest) {
  try {
    // Güvenlik: Cron secret key kontrolü
    const authHeader = request.headers.get('authorization')
    const cronSecret = process.env.CRON_SECRET || 'your-secret-key-change-this'

    if (authHeader !== `Bearer ${cronSecret}`) {
      return NextResponse.json(
        { error: 'Unauthorized' },
        { status: 401 }
      )
    }

    console.log('🔄 Çark hakkı yenilenme bildirimi başlıyor...')

    // Bildirim ayarını kontrol et
    const notifySetting = await prisma.settings.findUnique({
      where: { key: 'notify_wheel_reset' }
    })

    if (notifySetting?.value !== 'true') {
      console.log('⏭️ Bildirim ayarı kapalı')
      return NextResponse.json({
        success: false,
        message: 'Wheel reset notifications are disabled'
      })
    }

    // Günlük çark hakkı sayısını al
    const dailySpinsSetting = await prisma.settings.findUnique({
      where: { key: 'daily_wheel_spins' }
    })
    const dailySpins = Number.parseInt(dailySpinsSetting?.value || '3')

    // Tüm aktif kullanıcıları al (banlı olmayanlar ve telegramId'si olanlar)
    const users = await prisma.user.findMany({
      where: {
        isBanned: false,
        telegramId: { not: null }
      },
      select: {
        telegramId: true,
        firstName: true,
        username: true
      }
    })

    console.log(`📊 ${users.length} kullanıcıya bildirim gönderilecek`)

    let successCount = 0
    let failCount = 0

    // Kullanıcılara batch halinde mesaj gönder (Telegram rate limit'ini aşmamak için)
    for (let i = 0; i < users.length; i++) {
      const user = users[i]

      if (user.telegramId) {
        const success = await notifyWheelReset(user.telegramId, dailySpins)

        if (success) {
          successCount++
        } else {
          failCount++
        }

        // Her 30 mesajda bir 1 saniye bekle (Telegram rate limit)
        if ((i + 1) % 30 === 0) {
          console.log(`⏳ ${i + 1}/${users.length} mesaj gönderildi, kısa mola...`)
          await new Promise(resolve => setTimeout(resolve, 1000))
        } else if (i < users.length - 1) {
          // Normal delay - rate limit koruması
          await new Promise(resolve => setTimeout(resolve, 35))
        }
      }
    }

    console.log(`✅ Bildirim tamamlandı: ${successCount} başarılı, ${failCount} başarısız`)

    return NextResponse.json({
      success: true,
      totalUsers: users.length,
      successCount,
      failCount,
      message: `Wheel reset notifications sent to ${successCount} users`
    })
  } catch (error) {
    console.error('❌ Çark hakkı yenilenme bildirimi hatası:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
  }
}

// GET metodu ile bilgi al
export async function GET(request: NextRequest) {
  try {
    const authHeader = request.headers.get('authorization')
    const cronSecret = process.env.CRON_SECRET || 'your-secret-key-change-this'

    if (authHeader !== `Bearer ${cronSecret}`) {
      return NextResponse.json(
        { error: 'Unauthorized' },
        { status: 401 }
      )
    }

    // Ayarları kontrol et
    const [notifySetting, dailySpinsSetting] = await Promise.all([
      prisma.settings.findUnique({ where: { key: 'notify_wheel_reset' } }),
      prisma.settings.findUnique({ where: { key: 'daily_wheel_spins' } })
    ])

    const userCount = await prisma.user.count({
      where: {
        isBanned: false,
        telegramId: { not: null }
      }
    })

    return NextResponse.json({
      notificationsEnabled: notifySetting?.value === 'true',
      dailySpins: Number.parseInt(dailySpinsSetting?.value || '3'),
      activeUsers: userCount,
      endpoint: '/api/cron/wheel-reset',
      method: 'POST',
      authRequired: 'Bearer YOUR_CRON_SECRET'
    })
  } catch (error) {
    console.error('Error:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
  }
}
